/*
 * @Author: your name
 * @Date: 2021-12-31 15:55:18
 * @LastEditTime: 2022-01-11 22:05:14
 * @LastEditors: Please set LastEditors
 * @Description: 2-3-4树各类操作的实现部分
 * @FilePath: /datastructure/2_3_4tree/2_3_4tree.cpp
 */
#include<2-3-4tree.hpp>
/**
 * @description: B树的构造函数
 * @param {*}
 * @return {*}
...
std::pair<TreeNode*,int>  BTree::find(const int & value)
{
    ...
    while(!targetNode->isLeaf)
    {
        ...
        if(targetNode->valueList[i] == value)//如果在该结点中找到对应的关键字，返回该结点，以及其对应的索引
        {
            return{targetNode,i};
        }
...
void BTree::erase(const int & value)
{
    ...
    if(index != targetNode->nodeSize && targetNode->valueList[index] != value)
    {
        return;
    }
